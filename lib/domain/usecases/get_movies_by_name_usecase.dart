...
class GetMoviesByNameUsecase {
  ...
  GetMoviesByNameUsecase({required MoviesRepository moviesRepository, required TmdbRepository tmdbRepository})
  ...
  Future<Result<List<Movie>>> execute({required String name}) async {
    ...
    if (results case [
      ...
      Success<List<Movie>>(value: final moviesByName),
    ]) {
      final favoritesIDs = favorites.map((f) => f.id).toList(growable: false);
      return Success(moviesByName.markAsFavorite(favoritesIDs));
    }
