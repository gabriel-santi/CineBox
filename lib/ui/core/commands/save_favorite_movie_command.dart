...
class SaveFavoriteMovieCommand extends _$SaveFavoriteMovieCommand {
  ...
  Future<void> execute({
    ...
    final favoriteMovieCommand = ref.read(favoriteMovieCommandProvider(id).notifier);
    favoriteMovieCommand.setFavorite(true);

    final repo = ref.read(moviesRepositoryProvider);
    final result = await repo.saveFavoriteMovie(FavoriteMovie(id: id, title: title, posterPath: posterPath, year: year));

    switch (result) {
      case Success():
        log("Favorite adicionado com sucesso!");
